...
class Request():
    ...
    def path(self) -> str:
        if not hasattr(self, '_path'):
            self._path = self.request.split()[1]
        return self._path
...
class Response():
    "The content of an HTTP response is always considered to be a file"
    def __init__(self, content:str, cacheDurationSeconds: int = -1) -> None:
        pass
...
class HttpServer():
    def handleRequestInternal(self, clientSocket, address):
        ...
        handleRequestReturnVal = self.handleRequest() if isinstance(self.handleRequest, typing.Callable) else self.handleRequest
        handleRequest
        responseMsg = self.handleRequest()
