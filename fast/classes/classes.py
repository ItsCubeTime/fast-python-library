...
#                         self.setter = s1
#                         print(f"new setter of {self} is {self.setter}")
#                         self.setter(self, 1)
#                     else:
...
"""
    ...
    def __init__(self,
        ...
        if fset == None and fget.__code__.co_argcount > 1:
            ...
            if fset == None:
                def internalSetter(selfInner, value=None):
                    import inspect
                    if inspect.isfunction(selfInner):
                        self._setter = selfInner
                    else:
                        self._setter(value)
                    
                self.setter = self.__setAndGet__
            else:
                self.setter = fset
        self.deleter = fdel

    def __setAndGet__(self, value=_propertyNone):
        if not hasattr(self, 'value'):
            self.value = None
        if value != _propertyNone:
...
class User:
    def __init__(self, username: str, password: str, userId: str) -> None:
        "userId should be a string of any length thats guaranteed to be unique, contain only letters & numbers, no spaces or special characters."
        self.userId = userId
