...
# sys.path.insert(1, "C:/Users/olliv/Desktop/Art And Development/toga/toga src build/toga/winforms/src/")
# sys.path.insert(1, "C:/Users/olliv/Desktop/Art And Development/toga/toga src build/toga/core/src/")
...
# from toga_winforms import toga


# print(f"Controls: {self.native.Controls.__dir__()}\n\n")
# print(f"ControlBox: {[item for item in self.native.__dir__() if item.lower().__contains__('controlbox')]}\n\n")
# print(f"Yar!")
# print(f"{[item for item in self.native.__dir__() if item.lower().__contains__('text')]}")
# print(f"{[item for item in self.native.__dir__() if item.lower().__contains__('on')]}")
# print(f"onValidated: {self.native.OnValidated.__dir__()}\n\n")
# print(f"Controls: {self.native.Controls.__dir__()}\n\n")
# self.native.OnValidated=delayed
...
class TogaApp(toga.App):
    ...
    _startupTime = -1
    def __init__(self, formal_name=None, app_id=None, app_name=None, id=None, icon=data.appIcon, author=None, version=None, home_page=None, description=None, startup=None, windows=None, on_exit=None, factory=None):
        icon=data.appIcon
        print("icon: " + data.appIcon)
        ...
        def removeTitlebar(e=None, a=None):
            ...
            if False:
                ...
                def wndProc(hWnd, msg, wParam, lParam, *pack):
                    ...
                    if int(msg) in [ win32con.WM_SIZING,  win32con.WM_SIZE, win32con.WM_NCCALCSIZE]:
                    ...
                    # if int(msg) == win32con.WM_SIZE or int(msg) == win32con.SM_SHOWSOUNDS or int(msg) == win32con.SM_CXDOUBLECLK or int(msg) == 131 or int(msg) == 133 or int(msg) == 20:
                        ...
                        # import mouse
                        # if not mouse.is_pressed() and time.time() - self._startupTime > 20:
                            # print("early return")
                ...
                # extStyle = 0x00080000 # Transparent background
                # Steal begin
...
def applyNiceguiDesktopSpecificStyling(window: Window):
    ...
    with ui.card() as titlebar:
        ...
        with ui.row() as titlebarRow:
            ...
            with ui.row() as rightTitlebarRow:
                ...
                el.titlebar.button(lambda: window.set_minimized(not window.get_minimized()), "expand_more", transparentBackground=True)
                ui.image(serveFiles.serveFile(pathlib.Path(__file__).parent/'assets'/'minimize_208px.png')).style(add="width: 24px;").on("click", lambda: window.set_minimized(not window.get_minimized())).style(add="cursor:pointer;")
                ui.image(serveFiles.serveFile(pathlib.Path(__file__).parent/'assets'/'maximize_208px.png')).style(add="width: 24px;").on("click", lambda: window.set_maximized(not window.get_maximized())).style(add="cursor:pointer;")
                ui.image(serveFiles.serveFile(pathlib.Path(__file__).parent/'assets'/'close_208px.png')).style(add=   "width: 24px;").on("click", lambda: asyncronous.killProcess())                       .style(add="cursor:pointer;")
    ...
    nicegui.globals.get_client().content.default_slot.children.insert(0, ui.html('<div style="height: 48px;"></div>'))  # It looks like Im having some weird syntax error here
    ...
    # nicegui.globals.get_client().content.default_slot.parent.style(add="overflow: overlay; ")
    # nicegui.globals.get_client().content.default_slot.parent.parent_slot.parent.style(add="overflow: overlay; ")
    # nicegui.globals.get_client().content.default_slot.parent.parent_slot.parent.parent_slot.parent.style(add="overflow: overlay; ")
    # nicegui.globals.get_client().content.default_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.style(add="overflow: overlay; ")
    # nicegui.globals.get_client().content.default_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.style(add="overflow: overlay; ")
    # nicegui.globals.get_client().content.default_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.parent_slot.parent.style(add="overflow: overlay; ")
    ...
    .desktop { overflow: overlay;}
        ...
        pass
        nicegui.globals.get_client().content.default_slot.parent.style(add='width: 100vw; height: 100vh; overflow: hidden; flex-flow: column; align-items: flex-start; align-content: flex-start;')
...
def applyNiceguiUniversalStyling():
    from nicegui import ui
    ui.card().style(add='background-color:rgb(200,200,200); top:0; left:0; right:0; bottom:0; position:fixed; z-index:-1000;')
    js = '''
//js
...
class Window():  # Base class outlining API interface
    ...
    def __init__(self, adress: str = "0.0.0.0", port: int = 80, title: str = None, iconPath=None,
                 show_title_bar: bool = False, height= 500, width= 1000, commsPort=0, reloadOnChange=True, reloadOnChangePath: str = sys.argv[0], reloadOnChangeRestartWindow=True, hideNativeTitlebar=True, enableScrolling = False):
        ...
        togaApp = TogaApp(title, string.PascalCaseTo_snake_case(
            f"org.{data.publishers[0]}.{title}".replace(' ', '')))
