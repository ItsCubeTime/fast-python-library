...
class LocalStorageContainer():
    ...
    useCompression = True
    # folder = None
    def __init__(self, path: str|files.Folder) -> None:
        self.folder = files.Folder(str(path))
        self.folder.createFolder()
    def __new__(self, path: str|files.Folder):
        import inspect
...
class LocalStorage(LocalStorageContainer):
    def __init__(self, path: str|files.Folder, initialize=True) -> None:
        self._hasBeenInitialized = initialize
        self.folder = files.Folder(str(path))
        if initialize:
            super().__init__(path)
...
appdata = AppData(initialize=False)
